    ...
    */
    ...
    public function AdminGenerate()
    {
      ...
      //  $adm_file = fopen($path.'/../resources/views/back/layout.blade.php','w+');
        if (!file_exists($path.'/../resources/views/back/blocks/groupitems')){
            mkdir($path.'/../resources/views/back/blocks/groupitems/', 0777,true);
        }
    ...
    protected  function makeTitle($blockname,$fieldname){
        $template = config('Templates')['title'];
        $replaced = array('%name%','%type%','%block%','%value%');
        $replace  = array($fieldname, 'string', $blockname, "{{\$".$blockname."->title_field}}");
    ...
    protected function makeGroupImage($blockname,$groupname, $fieldname){

        $template = config('Templates')['group_image'];
        $replaced = array(
            '%name%',
            '%type%',
            '%block%',
    ...
    protected function makeGroupNumb($blockname,$groupname, $fieldname){
        $template = config('Templates')['group_numb'];
        $replaced = array(
            '%name%',
            '%type%',
            '%block%',
